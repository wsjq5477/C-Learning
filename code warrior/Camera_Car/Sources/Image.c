...
{     
    signed char i,j;
    unsigned char lost_line_count;
    unsigned char min,max;
    unsigned char left_down,right_down;
    int amount;
    int temp_black; 
    int temp_j;
    ...
    for(i=39;i>34;i--)                  //由近及远逐行提取,默认前5行不会出现赛道边沿部分
    {
        ...
        for(j=0;j<86;j++)              
        {
            if(image_two[i][j]==1&&image_two[i][j+1]==1&&image_two[i][j+2]==0&&image_two[i][j+3]==0)      //对黑线坐标取均值得到黑线中点
            {
                left_down = j;
            }        
        }
        ...
        {
            black_line[i] = (left_down+right_down)/2;
        }
        else
        {
            lost_line_count++;   
    ...
    for(i=34;i>-1;i--)                  //6-40行
    {
        temp_black = 0;
        amount = 0;
        if(black_line[i+1]-10>0)         //确保数组不越界
        {
            min = black_line[i+1]-10;            
        }
        else
        {
            min = 0;
        }
        if(black_line[i+1]+10<89)
        {
            max = black_line[i+1]+10;
        }
        else
        {
            max = 89;
        ...
        for(j=min;j<max;j++)            //每行从上一行中点的左右10个坐标内判断黑线,过滤噪点
        {    
            if(image_two[i][j]==1)
            {
                temp_j=(int)j;
